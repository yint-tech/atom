...
// 因体加密环境的授权组件开关，由 server 模块 gradle 构建注入
const ENABLE_AMS_NOTICE = import.meta.env.ENABLE_AMS_NOTICE === 'true';
...
const Topbar = (props: TopbarProps) => {
  ...
  useEffect(() => {
    if (!ENABLE_AMS_NOTICE) {
      return;
    }
    let noticeHook = (window as unknown as {
      _psyco1sdo1lb_?: () => void;
    })['_psyco1sdo1lb_'];
    if (noticeHook) {
      noticeHook();
