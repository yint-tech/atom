...
export interface SearchPanelField {
  ...
  uploadChange?: ChangeEventHandler<HTMLInputElement>;
}

interface SearchPanelProps {
  searchParam: Query;
  setSearchParam: (param: Query) => void;
...
const SearchPanel = (props: SearchPanelProps) => {
  ...
  // 渲染每个配置项
  const renderField = (fieldItem: SearchPanelField) => {
    const {
      component,
      label,
      field,
    ...
    if (component === 'select') {
      ...
      return (
        <Grid item xs={xs} key={field}>
          ...
          >
            {label}
            <VisuallyHiddenInput type='file' onChange={uploadChange} multiple />
          </Button>
        </Grid>
      );
    }
    ...
    <Card>
      ...
      <CardContent>
        {fields.map((row, rowIndex) => [
          <Grid container spacing={4} wrap='wrap' key={rowIndex}>
            {' '}
            {/* 使用稍大的间距 */}
            {row.map((field, fieldIndex) => (
              <Grid item xs={12} sm={6} md={4} lg={3} key={fieldIndex}>
                {' '}
                {/* 控制字段占位 */}
                {renderField(field)}
              </Grid>
          ...
          rowIndex < fields.length - 1 && (
            <Divider
              ...
              className={classes.dividerTopMargin}
            />
          ), // 添加顶部间距
        ])}
      </CardContent>
