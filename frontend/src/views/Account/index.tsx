...
const CreateUserDialog = (props: {
  ...
  setOpenCreateUserDialog: (open: boolean) => void;
...
const AccountList = () => {
  ...
  const [permOpAccount, setPermOpAccount] = useState<UserInfo>({} as UserInfo);
  const [showPermOpDialog, setShowPermOpDialog] = useState(false);
  const [refresh, setRefresh] = useState(+new Date());

  const travelToUser = (item: UserInfo) => {
    api
      .travelToUser({
        id: item.id,
      })
  ...
  const grantAdmin = (item: UserInfo) => {
    api
      .grantAdmin({
      ...
      <Container >
        ...
        columns={[
          ...
          },
          {
            ...
            render: (item: UserInfo) => (item.isAdmin ? <p>{t('common.yes')}</p> : <p>{t('common.no')}</p>),
          },
          {
