...
const parsePermsExp = (exp: string) => {
  let scope: string;
  let ret: string[] = [];
  exp.split('\n').forEach(line => {
    if (!line) {
...
function Permission(props: { account: UserInfo; setRefresh: (refresh: number) => void }) {
  const { t } = useTranslation();
  const { api } = useContext(AppContext);
  const { account, setRefresh } = props;
  const [permScopes, setPermScopes] = useState<string[]>([]);
  const [selectedPermScope, setSelectedPermScope] = useState('');

  const [allPermItems, setAllPermItems] = useState<string[]>([]);
  const [selectedPermItems, setSelectedPermItems] = useState<string[]>([]);
  const [availablePermItems, setAvailablePermItems] = useState<string[]>([]);

  const [userPermsExp, setUserPermExp] = useState('');
  const [userHoldPerms, setUserHolePerms] = useState<string[]>([]);
  ...
  const editPerm = (permItems: string[]) => {
    api
      .editUserPerm({
  ...
  const addPerm = (permItem: string) => {
    editPerm([...userHoldPerms, selectedPermScope + ':' + permItem]);
  };

  const removePerm = (permItem: string) => {
    editPerm(
