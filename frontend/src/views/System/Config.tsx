...
import { SettingItem } from '../../types/api';
...
function SingleInputItem({
  ...
  const doSave = () => {
    api.setConfig({ key: initKey, value: value as string }).then(res => {
      if (res.status === 0) {
...
const Form = () => {
  ...
  const theme = useTheme();
  const classes = useStyles({ theme });
  const [configs, setConfigs] = useState<SettingItem[]>([]);
                ...
                <div style={{ width: '100%' }}>
                  ...
                  <Grid container spacing={6} wrap='wrap'>
                    <SingleInputItem
                      ...
                      initKey={item.key}
                      initValue={item.value as string}
