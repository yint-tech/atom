...
import cn.iinti.atom.service.base.config.Configs.ConfigChangeListener
...
class MonitorThreadPoolExecutor : ThreadPoolExecutor {
    ...
    private fun setupMonitor() {
        registerGauge(
            "coreSize"
        ) { it: MonitorThreadPoolExecutor -> it.corePoolSize.toDouble() }
        registerGauge(
            "activeCount"
        ) { it: MonitorThreadPoolExecutor -> it.activeCount.toDouble() }
        registerGauge(
            "queueSize"
        ) { it: MonitorThreadPoolExecutor -> it.queue.size.toDouble() }
    ...
    private fun registerGauge(type: String, function: Function<MonitorThreadPoolExecutor, Double>) {
        Monitor.gauge(
            METRIC_NAME_GAUGE, Tags.of("name", name, "type", type),
            this
    ...
    private class MonitorRejectedExecutionHandler(private val delegate: RejectedExecutionHandler, threadName: String?) :
        ...
        private val rejectCounter: Counter = Monitor.counter(
            METRIC_NAME_COUNT, "name", threadName, "type", "reject"
        )
