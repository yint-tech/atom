package cn.iinti.katom.base.perm

import com.baomidou.mybatisplus.core.conditions.query.QueryWrapper

/**
 * atom的简单权限管理模型，只提供最简化的权限控制，即：作用域/权限
 * <br/>
 * 基于当代"逆中台化"的软件演化趋势，atom主要用户小作坊的小平台，提供最简化的中间件能力，权限控制也仅仅提供支持最基本的控制。
...
abstract class Permission<T> protected constructor(val clazz: Class<T>) {
     ...
     * @return 权限枚举
     */
    open fun perms(): Collection<String> {
        return emptyList()
    }
     ...
     */
    open fun filter(perms: Collection<String>, sql: QueryWrapper<T>) {
        // 默认为空实现
    }
     ...
     * @return 过滤后的返回列表
     */
    open fun filter(perms: Collection<String>, input: List<T>): List<T> {
        val filtered = mutableListOf<T>()
        for (t in input) {
            if (hasPermission(perms, t)) {
                filtered.add(t)
            }
        }
        return filtered
