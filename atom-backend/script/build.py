...
protection_main_jar = "atom-service"
protection_save_jars = ['mybatis-plus-extension']

deploy_remote_user = 'root'
deploy_server_list = ['atom.iinti.cn']
deploy_path = '/opt/atom'
# 若存在此配置，则构建脚本支持将发布包上传到oss，给用户下载
deploy_oss_server = 'oss.iinti.cn'
deploy_oss_asset_path = "/root/local-deplpy/gohttpserver/data/atom/"
...
def parse_constants(service_module_base):
    config_file = op.join(service_module_base, "src/main/java/cn/iinti/atom/service/base/env/Constants.java")
    with open(config_file, "r") as file:
        parse_ret = dict()
        for line in file.readlines():
            #  String docPath = "atom-doc";
...
def run_deploy(deploy_args):
    ...
    for server_node in deploy_server_list:
        ...
        execv(['scp', '-r', f"{project_root}/atom-service/src/main/resources-env/prod/",
...
def run_docker_build(docker_args):
    ...
    if deploy_oss_server and deploy_oss_asset_path:
        ...
        execv(['zip', '-f', 'atom-compose.zip', '.'])
        execv(['scp', 'atom-compose.zip', f"root@{deploy_oss_server}:{deploy_oss_asset_path}"])
        execv(['scp', op.join(iinti_asset_base, 'quickstart.sh'),
               f"root@{deploy_oss_server}:{deploy_oss_asset_path}"])
    os.chdir(docker_work_space)

    print('build docker compose img')
    img_prefix = f"{deploy_docker_registry}:atomserver"
    ...
    img_prefix = f"{deploy_docker_registry}:atom-allInOne"
...
service_module = op.join(project_root, "atom-service")
...
# fill default config
config['frontend_project'] = op.abspath(op.join(project_root, "../atom-frontend"))
...
parser = argparse.ArgumentParser(description="atom build script")
