...
(
    ...
    `accuracy`    varchar(8)  NOT NULL DEFAULT 'hours' COMMENT '精度，分为分钟、小时、天三个维度：minutes/hours/days',
    ...
    PRIMARY KEY (`id`),
    UNIQUE KEY `uniq_record` (`name`,`tags_md5`,`time_key`,`accuracy`),
    KEY           `idx_query` (`accuracy`,`name`,`tags_md5`),
    KEY `idx_delete` (`name`,`accuracy`,`create_time`) COMMENT '给删除指标使用'
) ENGINE=InnoDB AUTO_INCREMENT=0 DEFAULT CHARSET=utf8mb4  COMMENT='监控指标';
