...
public class ReflectUtil {
    private static final Map<ClassLoader, HashMap<String, Field>> fieldCache = new ConcurrentHashMap<>();
    private static final Map<ClassLoader, HashMap<String, Method>> methodCache = new ConcurrentHashMap<>();
    private static final Map<ClassLoader, HashMap<String, Constructor<?>>> constructorCache = new ConcurrentHashMap<>();
    ...
    public static HashMap<String, Field> getOrCreateFieldCache(ClassLoader classLoader) {
        return getOrCreateCache(classLoader, fieldCache);
    }

    public static HashMap<String, Method> getOrCreateMethodCache(ClassLoader classLoader) {
        return getOrCreateCache(classLoader, methodCache);
    }

    public static HashMap<String, Constructor<?>> getOrCreateConstructorCache(ClassLoader classLoader) {
        return getOrCreateCache(classLoader, constructorCache);
    }


    public static <T> HashMap<String, T> getOrCreateCache(ClassLoader classLoader, Map<ClassLoader, HashMap<String, T>> cache) {
        if (classLoader == null) {
            //jdk的class，对应classloader可能为null
            classLoader = ReflectUtil.class.getClassLoader();
        }
        HashMap<String, T> ret = cache.get(classLoader);
        if (ret != null) {
            return ret;
        }
        synchronized (ReflectUtil.class) {
            ret = cache.get(classLoader);
            if (ret != null) {
                return ret;
            }
            ret = new HashMap<>();
            cache.put(classLoader, ret);
            return ret;
        }
    }
    ...
    public static Method findMethodExact(Class<?> clazz, String methodName, Class<?>... parameterTypes) {
        ...
        HashMap<String, Method> classLoaderMethodHashMap = getOrCreateMethodCache(clazz.getClassLoader());

        if (classLoaderMethodHashMap.containsKey(fullMethodName)) {
            Method method = classLoaderMethodHashMap.get(fullMethodName);
            if (method == null)
        ...
        try {
            ...
            classLoaderMethodHashMap.put(fullMethodName, method);
            return method;
        } catch (NoSuchMethodException e) {
            classLoaderMethodHashMap.put(fullMethodName, null);
            throw new NoSuchMethodError(fullMethodName);
        }
    }
    ...
    public static Constructor<?> findConstructorExact(Class<?> clazz, Class<?>... parameterTypes) {
        ...
        HashMap<String, Constructor<?>> classLoaderConstructorCache = getOrCreateConstructorCache(clazz.getClassLoader());
    ...
    public static Field findField(Class<?> clazz, String fieldName) {
        ...
        HashMap<String, Field> classLoaderFieldCache = getOrCreateFieldCache(clazz.getClassLoader());
